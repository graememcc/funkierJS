module.exports = (function () {
  ...
  var decorate = function(f, arity, original, context) {
    ...
    define('apply', function(ctx, args) {
      if ((context !== undefined && ctx !== context) || (context === undefined && ctx === context))
        throw new Error('Cannot bind a curried function to a different execution context');

      return Function.prototype.apply.apply(this, [ctx].concat([args]));
    });

    define('call', function(ctx, args) {
      if ((context !== undefined && ctx !== context) || (context === undefined && ctx === context))
        throw new Error('Cannot bind a curried function to a different execution context');

      return Function.prototype.call.apply(this, [ctx].concat(args));
    });

    define('bind', function(ctx, args) {
      if ((context !== undefined && ctx !== context) || (context === undefined && ctx === context))
        throw new Error('Cannot bind a curried function to a different execution context');

      return Function.prototype.bind.apply(this, [ctx].concat(args));
    });
  ...
  var objectCurry = curry(function(fn) {
    return curryInternal({context: undefined}, arityOf(fn), fn);
  });
  ...
  // TODO: Revisit examples in this file once object functions implemented (use funkierJS equivalents
  //       rather than Object.create, [].slice, etc...)
  var objectCurryWithArity = curry(curryInternal.bind(null, {context: undefined}));
